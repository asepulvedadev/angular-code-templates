...
async function findAgentUrl(agentName) {
  try {
    ...
    const rootUrl = `https://raw.githubusercontent.com/asepulvedadev/angular-code-templates/main/cli-tool/components/agents/${agentName}.md`;
    ...
    const categoriesResponse = await fetch('https://api.github.com/repos/asepulvedadev/angular-code-templates/contents/cli-tool/components/agents');
    ...
    for (const item of contents) {
      if (item.type === 'dir') {
        const categoryUrl = `https://raw.githubusercontent.com/asepulvedadev/angular-code-templates/main/cli-tool/components/agents/${item.name}/${agentName}.md`;
        try {
  ...
  try {
    ...
    for (const item of contents) {
      if (item.type === 'file' && item.name.endsWith('.md')) {
        ...
        try {
          const categoryResponse = await fetch(`https://api.github.com/repos/asepulvedadev/angular-code-templates/contents/cli-tool/components/agents/${item.name}`);
          if (categoryResponse.ok) {
